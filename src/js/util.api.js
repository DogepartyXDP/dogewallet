...
function _makeJSONAPICall(destType, endpoints, method, params, timeout, onSuccess, onError, httpMethod) {
  /*Makes a JSON RPC API call to a specific dogepartyd/dogeblockd endpoint.
  ...
  //make JSON API call to dogeblockd
  if (destType == "dogeblockd") {
    makeJSONRPCCall(endpoints, method, params, timeout, onSuccess, onError);
  } else if (destType == "dogepartyd") {
    //make JSON API call to dogeblockd, which will proxy it to dogepartyd
    makeJSONRPCCall(endpoints, "proxy_to_dogepartyd", {
      "method": method,
      "params": params
...
function _getDestTypeFromMethod(method) {
  ...
  var destType = "dogepartyd";
  if (['is_ready', 'get_reflected_host_info', 'is_chat_handle_in_use', 'record_doge_open_order',
      'get_messagefeed_messages_by_index', 'get_normalized_balances', 'get_required_dogepays',
      ...
      'get_escrowed_balances', 'proxy_to_autodogeescrow', 'get_vennd_machine', 'get_script_pub_key', 'get_assets_info', 'broadcast_tx',
...
function supportUnconfirmedChangeParam(method) {
  return method.split("_").shift() == "create" && _getDestTypeFromMethod(method) == "dogepartyd";
}
      ...
      function(jqXHR, textStatus, errorThrown, endpoint) { //error callback
        ...
        if (gatheredResults.length == dwAPIUrls().length) {
          ...
          if (method != "is_ready") {
            ...
            //detect a special case of all servers returning code 525, which would mean dogepartyd had a reorg and/or we are upgrading
 ...
 * nonFailoverAPI: Used only by message feed requests currently (as the sequence numbers can theoritically change across multiple DW servers slightly due to mempool propagation differences)
 * failoverAPI: Used for all dogepartyd get_ API requests (for now...later we may want to move to multiAPINewest)
 * multiAPI: Used for storing dogeblockd state data (store_preferences, store_chat_handle, etc)
 * multiAPINewest: Used for fetching state data from dogeblockd (e.g. get_preferences, get_chat_handle)
 * multiAPIConsensus: Used for all dogepartyd create_ API requests
*/

function nonFailoverAPI(method, params, onSuccess, onError) {
  /*Make an API call to the "primary"" DW server (i.e. the first one in the shuffled servers list)
  ...
  */
  ...
  var destType = _getDestTypeFromMethod(method);
  _makeJSONAPICall(destType, dwAPIUrls()[0], method, params, TIMEOUT_FAILOVER_API, onSuccess, onErrorOverride);
}
...
function failoverAPI(method, params, onSuccess, onError) {
  ...
  */
  ...
  var destType = _getDestTypeFromMethod(method);
  _makeJSONAPICall(destType, dwAPIUrls(), method, params, TIMEOUT_FAILOVER_API, onSuccess, onErrorOverride);
}
  ...
  */
  ...
  if (typeof(onSysError) === 'undefined') {
    onSysError = function(jqXHR, textStatus, errorThrown, endpoint) {
      ...
      var noDogePos = textStatus.indexOf("Insufficient " + KEY_ASSET.DOGE);
      if (noDogePos != -1) {
        var endMessage = textStatus.indexOf(")", noDogePos) + 1;

        message = '<b class="errorColor">' + textStatus.substr(noDogePos, endMessage - noDogePos)
          + '</b>. You must have a small amount of ' + KEY_ASSET.DOGE + ' in this address to pay the Dogecoin miner fees. Please fund this address and try again.<br/><br/>'
          + '<a href="https://dogepartytalk.org/t/why-do-i-need-small-amounts-of-dogecoin-to-do-things/1142" target="_blank" rel="noopener noreferrer">More information on why this is necessary.</a>';
  ...
  _multiAPIPrimative(method, params, function(results) {
    ...
    if (isBech32(params['source'])) {
      ...
      if (!DWBitcore.compareOutputs(params['source'], successResults)) {
        return onConsensusError(successResults); //not all consensus data matches
      }
    }
    ...
    //if here, all is well
    if (onSuccess) {
      onSuccess(successResults[successResults.length - 1], dwAPIUrls().length, successResults.length);
    }
  });
}
