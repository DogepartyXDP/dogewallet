//Requires dwBitcore, USE_TESTNET
...
function createSharedKnockoutValidators() {
  ...
  ko.validation.rules['isValidDogecoinAddress'] = {
    validator: function(val, self) {
      return DWBitcore.isValidAddress(val) || DWBitcore.isValidMultisigAddress(val);
    },
    message: USE_TESTNET ? i18n.t('must_be_valid_testnet_address') : i18n.t('must_be_valid_dogecoin_address')
  };

  ko.validation.rules['isValidMonosigAddress'] = {
    validator: function(val, self) {
      return DWBitcore.isValidAddress(val);
    },
    message: USE_TESTNET ? i18n.t('must_be_valid_testnet_address') : i18n.t('must_be_valid_dogecoin_address')
  };

  ko.validation.rules['isValidDogecoinAddressIfSpecified'] = {
    validator: function(val, self) {
      try {
  ...
  ko.validation.rules['isNotSameDogecoinAddress'] = {
    validator: function(val, self) {
  ...
  ko.validation.rules['isValidUrlOrValidDogecoinAdressOrJsonBet'] = {
    validator: function(val, self) {
      if (!val) return false;
      // regex to check url, make freeze Chrome when checking doge address
      // TODO: change the way to check an url
      // return DWBitcore.isValidAddress(val) || isValidURL(val);
