//Dogewallet-specific utility functions
...
function decodeJsonBet(jsonBetBase64) {
  ...
  if (jsonBet.feed_address == undefined || !DWBitcore.isValidAddress(jsonBet.feed_address)) {
    return false;
  }
...
function pubkeyToPubkeyhash(pubkey) {
  return DWBitcore.pubKeyToPubKeyHash(pubkey);
}

function getPubkeyForAddress(address, callback) {
  if (!DWBitcore.isValidAddress(address) && !DWBitcore.isValidMultisigAddress(address)) {
    callback([]);
  }
  var pubkeys = [];
...
function getLinkForDPData(type, dataID, dataTitle, htmlize) {
