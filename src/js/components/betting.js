function FeedBrowserViewModel() {
  ...
  self.feedUrl = ko.observable('').extend({
    required: false,
    isValidUrlOrValidDogecoinAdressOrJsonBet: self
  });
  ...
  self.displayFeed = function(feed) {
    ...
    for (var i = 0; i < addresses.length; i++) {
      options.push({
        ...
        label: addresses[i][1] + ' (' + addresses[i][2] + ' ' + KEY_ASSET.XDP + ')'
      });
    ...
    for (var i in feed.counters.bets) {
      feed.counters.bets[i].wager_quantity = normalizeQuantity(feed.counters.bets[i].wager_quantity) + ' ' + KEY_ASSET.XDP;
      feed.counters.bets[i].wager_remaining = normalizeQuantity(feed.counters.bets[i].wager_remaining) + ' ' + KEY_ASSET.XDP;
    ...
    var onCounterbetsLoaded = function(data) {
      ...
      if (displayedData.length > 0) {
        ...
        for (var b = 0; b < displayedData2.length; b++) {
          ...
          displayedData2[b].volume_str = satoshiToXDP(displayedData2[b].countervolume);
          displayedData2[b].counterwager = satoshiToXDP(displayedData2[b].counterwager);
        }
      }
...
function OpenBetsViewModel() {
  ...
  self.displayOpenBets = function(data) {
    ...
    for (var i = 0; i < data.bets.length; i++) {
      ...
      bet.wager_quantity = satoshiToXDP(data.bets[i].wager_quantity);
      bet.wager_remaining = satoshiToXDP(data.bets[i].wager_remaining);
      bet.counterwager_quantity = satoshiToXDP(data.bets[i].counterwager_quantity);
      bet.counterwager_remaining = satoshiToXDP(data.bets[i].counterwager_remaining);
...
function MatchedBetsViewModel() {
  ...
  self.displayMatchedBets = function(data) {
    ...
    var genBetItem = function(data_bet, num_tx, feeds) {
      ...
      bet.wager = satoshiToXDP(bet.wager);
      bet.counterwager = satoshiToXDP(bet.counterwager);
