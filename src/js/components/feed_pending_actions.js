...
PendingActionViewModel.calcText = function(category, data) {
  ...
  if (category == 'burns') {
    ...
    desc = i18n.t("pend_or_unconf_send", pending, numberWithCommas(normalizeQuantity(data['quantity'], divisible)),
      asset_longname || data['asset'],
      getLinkForDPData('address', data['source'], getAddressLabel(data['source'])),
      getLinkForDPData('address', data['destination'], getAddressLabel(data['destination'])));
    ...
    if (data['transfer_destination']) {
      desc = i18n.t("pend_or_unconf_transfer", pending,
        asset_longname || data['asset'],
        getLinkForDPData('address', data['source'], getAddressLabel(data['source'])),
        getLinkForDPData('address', data['transfer_destination'], getAddressLabel(data['transfer_destination'])));
    } else if (data['locked']) {
    ...
    desc = i18n.t("pend_or_unconf_bet", pending, data['bet_type'], getLinkForDPData('address', data['feed_address'], getAddressLabel(data['feed_address'])),
  ...
  } else if (category == 'dogepays') {
    desc = i18n.t("pend_or_unconf_dogepay", pending, getAddressLabel(data['source']));
  } else if (category == 'order_matches') {

    if (WALLET.getAddressObj(data['tx1_address']) && data['forward_asset'] === KEY_ASSET.DOGE && data['_status'] === 'pending') {
      desc = i18n.t("pend_or_unconf_wait_dogepay", numberWithCommas(normalizeQuantity(data['forward_quantity'])), getAddressLabel(data['tx0_address']));
    } else if (WALLET.getAddressObj(data['tx0_address']) && data['backward_asset'] === KEY_ASSET.DOGE && data['_status'] === 'pending') {
      desc = i18n.t("pend_or_unconf_wait_dogepay", numberWithCommas(normalizeQuantity(data['backward_quantity'])), getAddressLabel(data['tx1_address']));
    }
...
function PendingActionFeedViewModel() {
  var self = this;
  self.entries = ko.observableArray([]); //pending actions beyond pending DOGEpays
  self.lastUpdated = ko.observable(new Date());
  self.ALLOWED_CATEGORIES = [
    'sends', 'dispensers', 'sweeps', 'orders', 'issuances', 'broadcasts', 'bets', 'dividends', 'burns', 'cancels', 'dogepays', 'order_matches'
  ...
  self.remove = function(txHash, category, dogeRefreshSpecialLogic) {
    if (typeof(dogeRefreshSpecialLogic) === 'undefined') dogeRefreshSpecialLogic = false;
    ...
    if (match) {
      //if the magically hackish dogeRefreshSpecialLogic flag is specified, then do a few custom checks
      // that prevent us from removing events whose txns we see as recent txns, but are actually NOT doge
      // send txns (e.g. is a dogeparty asset send, or asset issuance, or something the DOGE balance refresh
      // routine should NOT be deleting. This hack is a consequence of managing DOGE balances synchronously like we do)
      if (dogeRefreshSpecialLogic) {
        assert(category === "sends");
        if (match['CATEGORY'] !== category || match['DATA']['asset'] !== KEY_ASSET.DOGE)
          return;
...
PendingActionFeedViewModel.modifyBalancePendingFlag = function(category, data, flagSetting) {
  ...
  if (category == 'burns') {
    ...
    addressObj.getAssetObj(KEY_ASSET.XDP).balanceChangePending(flagSetting);
    updateUnconfirmedBalance(data['source'], KEY_ASSET.DOGE, data['quantity'] * -1);
    ...
    if (assetObj && assetObj.isMine()) {
      ...
      if (data['asset'].substring(0, 1) != 'A') {
        updateUnconfirmedBalance(data['source'], KEY_ASSET.XDP, -ASSET_CREATION_FEE_XDP * UNIT);
      }
    }
    ...
    if (data['give_asset'] !== KEY_ASSET.DOGE) {
      updateUnconfirmedBalance(data['source'], data['give_asset'], data['give_quantity'] * -1);
    }

  } else if (category == 'bets') {

    updateUnconfirmedBalance(data['source'], KEY_ASSET.XDP, data['wager_quantity'] * -1);
