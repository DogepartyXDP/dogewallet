function WalletViewModel() {
  //The user's wallet
  var self = this;
  self.DOGECOIN_WALLET = null; // DWHierarchicalKey instance
  self.autoRefreshDOGEBalances = true; //auto refresh DOGE balances every 5 minutes

  self.identifier = ko.observable(null); //set when logging in
  self.networkBlockHeight = ko.observable(null); //stores the current network block height. refreshed when we refresh the DOGE balances
  ...
  self.isSellingDOGE = ko.observable(false); //updated by the dogepay feed
  ...
  self.addAddress = function(type, address, pubKeys) {
    ...
    if (type == 'normal') {
      ...
      var key = self.DOGECOIN_WALLET.getAddressKey(i);
  ...
  self.getAddressesList = function(withLabel) {
    ...
    ko.utils.arrayForEach(self.addresses(), function(address) {
      if (withLabel) {
        addresses.push([address.ADDRESS, address.label(), address.getXDPBalance(), address.PUBKEY]);
      } else {
  ...
  self.getBiggestXDPBalanceAddress = function() {
    var maxAmount = 0;
    var maxAddress = null;

    ko.utils.arrayForEach(self.addresses(), function(address) {
      var xdpBalance = address.getXDPBalance();
      if (xdpBalance > maxAmount) {
        maxAmount = xdpBalance;
        maxAddress = address;
      }
    });
  ...
  self.updateBalance = function(address, asset, rawBalance, unconfirmedRawBal) {
    ...
    var assetObj = addressObj.getAssetObj(asset);
    if (!assetObj) {
      assert(asset !== KEY_ASSET.XDP && asset !== KEY_ASSET.DOGE, [KEY_ASSET.DOGE, 'or', KEY_ASSET.XDP, 'not present in the address?'].join(' ')); //these should be already in each address
      ...
      if (asset === KEY_ASSET.DOGE && unconfirmedRawBal) {
  ...
  self.getAssetsDivisibility = function(assets, callback) {
    ...
    if (notAvailable.length > 0) {
      // else make a query to dogepartyd
  ...
  self.refreshDogepartyBalances = function(addresses, onSuccess) {
    //update all dogeparty asset balances for the specified address (including XDP)
    //Note: after login, this normally never needs to be called (except when adding a watch address),
    // as dogeparty asset balances are updated automatically via the messages feed
      ...
      function(balancesData, endpoint) {
        ...
        if (!balancesData.length) {
          for (var i in addresses) {
            WALLET.getAddressObj(addresses[i]).addOrUpdateAsset(KEY_ASSET.XDP, {}, 0, 0);
          }
        ...
        // TODO: optimize: assets infos already fetched in get_normalized_balances() in dogeblockd
  ...
  self.refreshDOGEBalances = function(isRecurring, addresses, onSuccess) {
    if (typeof(isRecurring) === 'undefined') isRecurring = false;
    //^ if isRecurring is set to true, we will update DOGE balances every 5 min as long as self.autoRefreshDOGEBalances == true

    //update all DOGE balances (independently, so that one addr with a bunch of txns doesn't hold us up)
    ...
    //See if we have any pending DOGE send transactions listed in Pending Actions, and if so, enable some extra functionality
    // to clear them out if we sense the txn as processed
    var pendingActionsHasDOGESend = ko.utils.arrayFirst(PENDING_ACTION_FEED.entries(), function(item) {
      return item.CATEGORY === 'sends' && item.DATA['asset'] === KEY_ASSET.DOGE; //there is a pending DOGE send
    });

    self.retrieveDOGEAddrsInfo(addresses, function(data) {
      ...
      for (var i = 0; i < data.length; i++) {
        //if someone sends DOGE using the wallet, an entire TXout is spent, and the change is routed back. During this time
        // the (confirmed) balance will be decreased by the ENTIRE quantity of that txout, even though they may be getting
        // some/most of it back as change. To avoid people being confused over this, with DOGE in particular, we should
        // display the unconfirmed portion of the balance in addition to the confirmed balance, as it will include the change output
        self.updateBalance(data[i]['addr'], KEY_ASSET.DOGE, data[i]['confirmedRawBal'], data[i]['unconfirmedRawBal']);
        self.updateDispensers(data[i]['addr'])

        addressObj = self.getAddressObj(data[i]['addr']);
        assert(addressObj, 'Cannot find address in wallet for refreshing ' + KEY_ASSET.DOGE + ' balances!');
        ...
        if (data[i]['confirmedRawBal'] && !addressObj.IS_WATCH_ONLY) {
          //Also refresh DOGE unspent txouts (to know when to "reprime" the account)
          ...
          $.jqlog.debug("refreshDOGEBalances: Address " + data[i]['addr'] + " -- confirmed bal = " + data[i]['confirmedRawBal']
          ...
          if (pendingActionsHasDOGESend) {
            //see if data[i]['lastTxns'] includes any hashes that exist in the Pending Actions, which
            // means we MAY be able to remove them from that listing (i.e. they COULD be non-DOGE send (i.e. dogeparty transactions) though
            //TODO: This is not very efficient when a DOGE send is pending... O(n^3)! Although the sample sets are relatively small...
      ...
      if (isRecurring && self.autoRefreshDOGEBalances) {
        setTimeout(function() {
          if (self.autoRefreshDOGEBalances) { self.refreshDOGEBalances(true); }
        }, 60000 * 5);
      }
      ...
      //system down or spazzing, set all DOGE balances out to null
      var addressObj = null;
      for (var i = 0; i < addresses.length; i++) {
        self.updateBalance(addresses[i], KEY_ASSET.DOGE, null, null); //null = UNKNOWN
        addressObj = self.getAddressObj(addresses[i]);
      ...
      $.jqlog.warn(i18n.t("doge_sync_error", textStatus));
      //bootbox.alert(i18n.t("doge_sync_error", textStatus));
  ...
  self.removeKeys = function() {
    ...
    //stop DOGE balance timer on each address
    ko.utils.arrayForEach(self.addresses(), function(a) {
      a.doDOGEBalanceRefresh = false;
    });
  ...
  self.signAndBroadcastTxRaw = function(key, unsignedTxHex, onSuccess, onError, verifySourceAddr, verifyDestAddr) {
    ...
    //Sign and broadcast a multisig transaction that we got back from dogepartyd (as a raw unsigned tx in hex)
    ...
    if (hasAnyBech32) {
      ...
      var network = bitcoinjs.networks[self.DOGECOIN_WALLET.HierarchicalKey.network.alias];
      ...
      self.retrieveDOGEAddrsInfo([verifySourceAddr], function(data) {
        var utxoMap = {}
  ...
  self.retrieveDOGEBalance = function(address, onSuccess, onError) {
    //We used to have a retrieveDOGEBalances function for getting balance of multiple addresses, but scrapped it
    ...
    failoverAPI("get_chain_address_info", {"addresses": [address], "with_uxtos": false, "with_last_txn_hashes": 0},
      function(data, endpoint) {
        return onSuccess(
          parseInt(data[0]['info']['balanceSat'] || 0), //confirmed DOGE balance
          parseInt(data[0]['info']['unconfirmedBalanceSat'] || 0) //unconfirmed DOGE balance
        );
      },
      onError || defaultErrorHandler);
  }

  self.retrieveDOGEAddrsInfo = function(addresses, onSuccess, onError, minConfirmations) {
    ...
    failoverAPI("get_chain_address_info", {"addresses": addresses, "with_uxtos": true, "with_last_txn_hashes": 5},
      function(data, endpoint) {
        ...
        //final number of primed txouts is lesser of either the # of txouts that are >= .0005 DOGE, OR the floor(total balance / .0005 DOGE)
  ...
  /////////////////////////
  //Dogeparty transaction-related
  self.canDoTransaction = function(address) {
    /* ensures that the specified address can perform a dogeparty transaction */
    var addressObj = self.getAddressObj(address);
    assert(!addressObj.IS_WATCH_ONLY, "Cannot perform this action on a watch only address!");

    if (self.getBalance(address, KEY_ASSET.DOGE, false) < MIN_BALANCE_FOR_ACTION) {
      bootbox.alert(i18n.t("insufficient_doge", normalizeQuantity(MIN_BALANCE_FOR_ACTION), getAddressLabel(address)));
      return false;
    }
