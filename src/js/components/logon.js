function WalletCreationViewModel() {
  ...
  self.quickAccessUrl = ko.computed(function() {
    if (self.generatedPassphrase().length > 0 && self.quickAccessPassword().length > 0) {
      return DWBitcore.getQuickUrl(self.generatedPassphrase(), self.quickAccessPassword());
    }
  });
...
function LogonViewModel() {
  ...
  self.decryptEnteredPassphrase = function() {
    ...
    try {
      var decryptedPassphrase = DWBitcore.decrypt(self.cryptedPassphrase(), self.passwordDecrypt());
  ...
  self.openWallet = function() {
    ...
    multiAPI("is_ready", {}, function(data, endpoint) {
        ...
        $.jqlog.log("Backend is ready. Testnet status: " + USE_TESTNET + ". Last message feed index: " + data['last_message_index'] + ". DW last message seq: " + data['dw_last_message_seq']);
        ...
        //Set initial block height (will be updated again on each periodic refresh of DOGE account balances)
        WALLET.networkBlockHeight(data['block_height']);

        //Initialize the message feed (polls the server and notifies us of new
        //events, as dogeparty processes confirmed blocks and tx in mempool)
        MESSAGE_FEED.init(data['last_message_index'], data['dw_last_message_seq']);
      ...
      function(jqXHR, textStatus, errorThrown, endpoint) {
        ...
        bootbox.alert(i18n.t("no_dogeparty_error", message));
      });
  }
  ...
  self.openWalletPt2 = function(mustSavePreferencesToServer) {
    ...
    WALLET.DOGECOIN_WALLET = new DWHierarchicalKey(self.enteredPassphrase());
    WALLET.isOldWallet(WALLET.DOGECOIN_WALLET.useOldHierarchicalKey);
  ...
  self.genAddress = function(mustSavePreferencesToServer, addressCount) {
    ...
    WALLET.refreshDOGEBalances(false, moreAddresses, function() {
  ...
  self.updateBalances = function(additionalDOGEAddresses, onSuccess) {
    //updates all balances for all addesses, creating the asset objects on the address if need be
    WALLET.refreshDOGEBalances(true, additionalDOGEAddresses, function() {
      //^ specify true here to start a recurring get DOGE balances timer chain
      WALLET.refreshDogepartyBalances(WALLET.getAddressesList(), onSuccess);
    });
  }

  self.openWalletPt3 = function(mustSavePreferencesToServer) {
    //add in the armory and watch only addresses
    var additionalDOGEAddresses = [], i = null;
    for (i in PREFERENCES['armory_offline_addresses']) {
      try {
        ...
        additionalDOGEAddresses.push(PREFERENCES['armory_offline_addresses'][i]['address']);
      } catch (e) {
    ...
    for (i in PREFERENCES['watch_only_addresses']) {
      try {
        ...
        additionalDOGEAddresses.push(PREFERENCES['watch_only_addresses'][i]);
      } catch (e) {
    ...
    for (i in PREFERENCES['multisig_addresses']) {
      try {
        ...
        additionalDOGEAddresses.push(PREFERENCES['multisig_addresses'][i]['address']);
      } catch (e) {
    ...
    self.updateBalances(additionalDOGEAddresses, self.openWalletPt4);
  }
