function BalanceHistoryViewModel() {
  ...
  self.availableAssets = !USE_TESTNET ? ko.observableArray([KEY_ASSET.XDP, KEY_ASSET.DOGE]) : ko.observableArray([KEY_ASSET.XDP]);
  //^ don't load DOGE as an asset on testnet, since we can't show the data (since blockchain doesn't support testnet)
  self.graphData = null;
  ...
  self.init = function() {
    ...
    failoverAPI("get_owned_assets", {addresses: WALLET.getAddressesList()}, function(data, endpoint) {
      ...
      //^ this way, XDP and DOGE stay at the top, but the other assets are alphabetically sorted
    });
  ...
  self.assetChanged = function() {
    ...
    if (self.selectedAsset() === KEY_ASSET.DOGE) { //mainnet only (as we use blockchain.info for this and they don't support testnet)
      var addresses = WALLET.getAddressesList();
      self.graphData = [];

      for (var i = 0; i < addresses.length; i++) {
        //since we don't track DOGE balances, we need to go to blockchain.info for that
        ...
        $.queryYQL(q, function(data, textStatus, jqXHR) {
          ...
          var address = /address%3D([A-Za-z0-9]+)%22/g.exec(jqXHR.url)[1];
          var addressHash = hashToB64(address);
          $.jqlog.debug('Could not get ' + KEY_ASSET.DOGE + ' balance from blockchain for address ' + address + ': ' + errorThrown);
          var addressName = PREFERENCES['address_aliases'][addressHash] ? "<b>" + PREFERENCES['address_aliases'][addressHash] + "</b> (" + address + ")" : address;
...
function TransactionHistoryItemViewModel(data) {
  ...
  self.dispSource = function() {
    return getLinkForDPData('address', self.SOURCE, getAddressLabel(self.SOURCE));
  }

  //self.dogeQuantity = TODO
  //self.fee = TODO
  ...
  self.dispDescription = function() {
    ...
    if (self.RAW_TX_TYPE == 'burns') {
      ...
      desc = i18n.t("hist_send", smartFormat(normalizeQuantity(self.DATA['quantity'],
        ...
        getLinkForDPData('address', self.DATA['destination'], getAddressLabel(self.DATA['destination'])));
      ...
      if (self.DATA['forward_asset'] === KEY_ASSET.DOGE || self.DATA['backward_asset'] === KEY_ASSET.DOGE) {
        desc += " <b>(" + i18n.t("pending DOGEpay") + ")</b>";
      }
    } else if (self.RAW_TX_TYPE == 'dogepays') {
      desc = i18n.t("hist_dogepay", smartFormat(normalizeQuantity(self.DATA['doge_amount'])));
      ...
      if (self.DATA['transfer']) {
        desc = i18n.t("hist_transfer", self.DATA['_asset_longname'] || self.DATA['asset'],
          getLinkForDPData('address', self.DATA['issuer'], getAddressLabel(self.DATA['issuer'])));
      ...
      desc = i18n.t("hist_bet", BET_TYPES[self.DATA['bet_type']],
        getLinkForDPData('address', self.DATA['feed_address'], getAddressLabel(self.DATA['feed_address'])),
      ...
      desc = i18n.t("hist_bet_match", getLinkForDPData('address', self.DATA['feed_address'], getAddressLabel(self.DATA['feed_address'])),
      ...
      desc = i18n.t("hist_credit_debit", getLinkForDPData('address', self.DATA['address'], getAddressLabel(self.DATA['address'])), tx_type,
