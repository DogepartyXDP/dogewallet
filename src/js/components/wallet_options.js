function WalletOptionsModalViewModel() {
  ...
  self.autoDOGEPayEnabled = ko.observable(null);
  ...
  self.walletUrl = ko.computed(function() {
    if (self.urlPassword().length > 0 && WALLET.DOGECOIN_WALLET) {
      return WALLET.DOGECOIN_WALLET.getQuickUrl(self.urlPassword());
    }
  });
  ...
  self.dispDWURLS = ko.computed(function() {
    return dwURLs() ? dwURLs().join(', ') : i18n.t('unknown');
  }, self);
