...
function CreateNewAddressModalViewModel() {
  ...
  self.multisigAddress1.subscribe(function(val) {
    if (val && DWBitcore.isValidAddress(val)) {
  ...
  self.doAction = function() {
    ...
    WALLET.storePreferences(function(data, endpoint) {
      WALLET.refreshDogepartyBalances([newAddress], function() {
        WALLET.refreshDOGEBalances(false, null, function() {
          self.shown(false);
...
function SendModalViewModel() {
  ...
  self.destAddress = ko.observable('').extend({
    required: true,
    isValidDogecoinAddress: self,
    isNotSameDogecoinAddress: self
  });
  ...
  self.destAddress.subscribe(function(val) {
    ...
    if (!DWBitcore.isValidMultisigAddress(self.destAddress())) return;

    getPubkeyForAddress(val, function(data) {
      var addresses = []
      if (DWBitcore.isValidMultisigAddress(self.destAddress())) {
  ...
  shouldShowMemoFields = ko.computed(function() {
    return self.asset() && self.asset() !== KEY_ASSET.DOGE;
  })
  ...
  self.feeController = DWFeeModelMixin(self, {
    action: "create_send",
  ...
  self.show = function(fromAddress, asset, assetDisp, rawBalance, isDivisible, resetForm) {
    if (asset === KEY_ASSET.DOGE && rawBalance === null) {
...
var privateKeyValidator = function(required) {
  return {
    ...
    validation: {
      validator: function(val, self) {
        return (new DWPrivateKey(val)).isValid();
      },
...
function SweepModalViewModel() {
  var self = this;
  self.shown = ko.observable(false);
  self.notEnoughDOGE = ko.observable(false);
  ...
  self.selectedAssetsToSweep = ko.observableArray([]).extend({
    ...
    validation: {
      validator: function(val, self, callback) {
        ...
        // here we assume that the transaction cost to send DOGE from addressForFees is MIN_FEE
        var totalDogeBalanceForSweeep = self.dogeBalanceForPrivateKey() + Math.max(0, (self.addressForFeesBalance() - MIN_FEE));
        self.missingDogeForFees = Math.max(MULTISIG_DUST_SIZE, sweepingCost - self.dogeBalanceForPrivateKey());


        if (totalDogeBalanceForSweeep < sweepingCost) {

          this.message = i18n.t("not_able_to_sweep", normalizeQuantity(self.missingDogeForFees), self.addressForPrivateKey());
          self.notEnoughDOGE(true);
          return false;

        } else if (self.dogeBalanceForPrivateKey() >= sweepingCost) {
          self.privateKeyForFees('');
          self.addressForFeesBalance(0);
        }

        self.notEnoughDOGE(false);
        return true;
      },
      params: self
    }
  ...
  self.addressForPrivateKey = ko.computed(function() {
    ...
    return (new DWPrivateKey(self.privateKey())).getAddress();
  }, self);
  ...
  self.addressForPrivateKeyForFees = ko.computed(function() {
    ...
    //Get the address for this privatekey
    return (new DWPrivateKey(self.privateKeyForFees())).getAddress();
  }, self);

  self.dogeBalanceForPrivateKey = ko.observable(0);
  ...
  self.addressForPrivateKey.subscribe(function(address) {
    ...
    var hash = {};
    hash[KEY_ASSET.DOGE] = MIN_FEE + REGULAR_DUST_SIZE;
    ...
    failoverAPI("get_normalized_balances", {'addresses': [address]}, function(balancesData, endpoint) {
      ...
      failoverAPI("get_assets_info", {'assetsList': assets}, function(assetsData, endpoint) {
        ...
        //Also get the DOGE balance at this address and put at head of the list
        //We just check if unconfirmed balance > 0.
        WALLET.retrieveDOGEAddrsInfo([address], function(data) {
          self.dogeBalanceForPrivateKey(0);
          self.txoutsCountForPrivateKey = 0;
          //TODO: dogeblockd return unconfirmedRawBal==0, after fixing we need use unconfirmedRawBal
          var unconfirmedRawBal = data[0]['confirmedRawBal'];
          if (unconfirmedRawBal > 0) {
            //We don't need to supply asset info to the SweepAssetInDropdownItemModel constructor for DOGE
            // b/c we won't be transferring any asset ownership with it
            var viewModel = new SweepAssetInDropdownItemModel(KEY_ASSET.DOGE, unconfirmedRawBal, normalizeQuantity(unconfirmedRawBal));
            self.availableAssetsToSweep.unshift(viewModel);
            assets.push(KEY_ASSET.DOGE);
            self.dogeBalanceForPrivateKey(data[0]['confirmedRawBal']);
  ...
  self.addressForPrivateKeyForFees.subscribe(function(address) {
    ...
    WALLET.retrieveDOGEAddrsInfo([address], function(data) {
      self.addressForFeesBalanceMessage([normalizeQuantity(data[0]['confirmedRawBal']), KEY_ASSET.DOGE, 'in', address].join(' '));
  ...
  self.resetForm = function(fromOldWallet) {
    ...
    if (self.fromOldWallet()) {
      WALLET.DOGECOIN_WALLET.getOldAddressesInfos(function(data) {
        for (var address in data) {
          if (self.excludedOldAddresses.indexOf(address) == -1) {
            self.availableOldAddresses.push(new BalancesAddressInDropdownItemModel(address, address, data[address][KEY_ASSET.DOGE]['privkey']));
          }
        }
      });
    }
  ...
  self.waitTxoutCountIncrease = function(callback) {
    setTimeout(function() {
      WALLET.retrieveDOGEAddrsInfo([self.addressForPrivateKey()], function(data) {
  ...
  self.sendDogeForFees = function(callback) {
    var dwk = new DWPrivateKey(self.privateKeyForFees());
    var pubkey = dwk.getPub();

    // if address has one ouptut, it will has two after this transaction..
    // ..so need output merging
    if (self.txoutsCountForPrivateKey == 1) {
      self.missingDogeForFees += 2 * MIN_FEE;
    }
    // To avoid "Destination output is below the dust target value" error
    var sweepDOGE = false;
    for (var i = 0; i < self.selectedAssetsToSweep().length; i++) {
      var assetName = self.selectedAssetsToSweep()[i];
      if (assetName === KEY_ASSET.DOGE) sweepDOGE = true;
    }
    if (sweepDOGE) {
      self.missingDogeForFees += REGULAR_DUST_SIZE;
    }
    $.jqlog.debug('missingDogeForFees: ' + self.missingDogeForFees);

    var sendData = {
      source: self.addressForPrivateKeyForFees(),
      destination: self.addressForPrivateKey(),
      quantity: self.missingDogeForFees,
      asset: KEY_ASSET.DOGE,
      encoding: 'auto',
      pubkey: pubkey,
    ...
    var onTransactionBroadcasted = function(sendTxHash, endpoint) { //broadcast was successful
      ...
      var newBalance = self.dogeBalanceForPrivateKey() + self.missingDogeForFees;
      self.dogeBalanceForPrivateKey(newBalance);
    ...
    var onTransactionCreated = function(unsignedTxHex, numTotalEndpoints, numConsensusEndpoints) {
      dwk.checkAndSignRawTransaction(unsignedTxHex, [self.addressForPrivateKey()], function(err, signedHex) {
        if (err) {
    ...
    var message = i18n.t("sending_doge_for_sweeping_fees", normalizeQuantity(self.missingDogeForFees), self.addressForPrivateKeyForFees());
    self.sweepingProgressionMessage(message);
    $.jqlog.debug(message);
  ...
  // so the final balance for doge transfert is the value of last change that we get with extractChangeTxoutValue()
  // TODO: think for a more economic way to have a reliable amount for the final tx (DOGE).
  self.mergeOutputs = function(key, pubkey, callback, fees) {
    if (self.txoutsCountForPrivateKey > 1) {
      ...
      var sendData = {
        ...
        destination: self.addressForPrivateKey(),
        quantity: self.dogeBalanceForPrivateKey() - fees,
        asset: KEY_ASSET.DOGE,
        encoding: 'auto',
        pubkey: pubkey,
        allow_unconfirmed_inputs: true,
      ...
      var onTransactionError = function() {
        if (arguments.length == 4) {
          ...
          if (match != null) {
            ...
            // if insufficient dogecoins we retry with estimated fees return by dogepartyd
            var minEstimateFee = denormalizeQuantity(parseFloat(match[1])) - (self.dogeBalanceForPrivateKey() - self.mergeCost);
            $.jqlog.debug('Insufficient fees. Need approximately ' + normalizeQuantity(minEstimateFee));

            if (minEstimateFee > self.dogeBalanceForPrivateKey()) {
              self.shown(false);
              bootbox.alert(arguments[1]);
            } else {
      ...
      function(unsignedTxHex, numTotalEndpoints, numConsensusEndpoints) {
        ...
        key.checkAndSignRawTransaction(unsignedTxHex, [self.destAddress()], function(err, signedHex) {
          ...
          WALLET.broadcastSignedTx(signedHex, function(issuanceTxHash, endpoint) { //broadcast was successful
            ...
            // here we adjust the DOGE balance whith the change output
            var newDogeBalance = DWBitcore.extractChangeTxoutValue(transferData.source, unsignedTxHex);
            $.jqlog.debug(['New', KEY_ASSET.DOGE, 'balance:', newDogeBalance].join(' '));
            self.dogeBalanceForPrivateKey(newDogeBalance);
  ...
  self._doSendAsset = function(asset, key, pubkey, opsComplete, adjustedDOGEQuantity, callback) {
    $.jqlog.debug('_doSendAsset: ' + asset);

    //TODO: remove this
    if (asset === KEY_ASSET.DOGE) assert(adjustedDOGEQuantity !== null);
    else assert(adjustedDOGEQuantity === null);
    ...
    $.jqlog.debug("dogeBalanceForPrivateKey: " + self.dogeBalanceForPrivateKey());
    var quantity = (asset === KEY_ASSET.DOGE) ? (self.dogeBalanceForPrivateKey() - MIN_FEE) : selectedAsset.RAW_BALANCE;
    var normalizedQuantity = (asset === KEY_ASSET.DOGE) ? normalizeQuantity(quantity) : selectedAsset.NORMALIZED_BALANCE;

    assert(selectedAsset);

    if (!quantity) { //if there is no quantity to send for the asset, only do the transfer
      if (asset === KEY_ASSET.XDP || asset === KEY_ASSET.DOGE) { //nothing to send, and no transfer to do
    ...
    multiAPIConsensus("create_send", sendData, //can send both DOGE and dogeparty assets
      function(unsignedTxHex, numTotalEndpoints, numConsensusEndpoints) {
        ...
        key.checkAndSignRawTransaction(unsignedTxHex, [self.destAddress()], function(err, signedHex) {
          ...
          WALLET.broadcastSignedTx(signedHex, function(sendTxHash, endpoint) { //broadcast was successful
            ...
            // here we adjust the DOGE balance whith the change output
            if (selectedAsset.ASSET !== KEY_ASSET.DOGE) {
              var newDogeBalance = DWBitcore.extractChangeTxoutValue(sendData.source, unsignedTxHex);
              $.jqlog.debug(['New', KEY_ASSET.DOGE, 'balance:', newDogeBalance].join(' '));
              self.dogeBalanceForPrivateKey(newDogeBalance);
            }

            //For non DOGE/XDP assets, also take ownership (iif the address we are sweeping from is the asset's owner')
            if (selectedAsset.ASSET !== KEY_ASSET.XDP
              && selectedAsset.ASSET !== KEY_ASSET.DOGE
  ...
  self.doAction = function() {
    var dwk = new DWPrivateKey(self.privateKey());
    var pubkey = dwk.getPub();

    var sendsToMake = [];
    var opsComplete = [];

    var selectedAsset = null, hasDOGE = false;
    for (var i = 0; i < self.selectedAssetsToSweep().length; i++) {
      selectedAsset = self.selectedAssetsToSweep()[i];
      if (selectedAsset === KEY_ASSET.DOGE) {
        hasDOGE = i; //send DOGE last so the sweep doesn't randomly eat our primed txouts for the other assets
      } else {
        sendsToMake.push([selectedAsset, dwk, pubkey, opsComplete, null]);
      }
    }
    if (hasDOGE !== false) {
      //This balance is adjusted after each asset transfert with the change output.
      sendsToMake.push([KEY_ASSET.DOGE, dwk, pubkey, opsComplete, self.dogeBalanceForPrivateKey()]);
    }
    ...
    var launchSweep = function() {
      if (sendsToMake.length === 1 && sendsToMake[0][0] === KEY_ASSET.DOGE) {
        doSweep();
      } else {
        // merge output then start sweeping.
        self.mergeOutputs(dwk, pubkey, doSweep);
      }
    }

    trackEvent('Balances', self.fromOldWallet() ? 'SweepFromOldWallet' : 'Sweep');

    if (self.missingDogeForFees > 0 && self.privateKeyForFeesValidated.isValid() != '') {
      // send doge to pay fees then launch sweeping
      self.sendDogeForFees(launchSweep);
    } else {
      launchSweep();
    }
  }
...
function TestnetBurnModalViewModel() {
  ...
  self.dogeAlreadyBurned = ko.observable(null); // quantity DOGE already burned from this address (normalized)

  self.dogeBurnQuantity = ko.observable('').extend({
    required: true,
    isValidPositiveQuantity: self,
    validation: [{
      validator: function(val, self) {
        return parseFloat(val) >= 1 && parseFloat(val) <= 1000000;
      },
      message: i18n.t('quantity_must_be_between_0_and_1'),
      params: self
    }, {
      validator: function(val, self) {
        return parseFloat(val) <= WALLET.getBalance(self.address(), KEY_ASSET.DOGE) - normalizeQuantity(MIN_FEE);
      },
      message: i18n.t('quantity_of_exceeds_balance', KEY_ASSET.DOGE),
      params: self
    }, {
      validator: function(val, self) {
  ...
  self.quantityXDPToBeCreated = ko.computed(function() { //normalized
    if (!self.dogeBurnQuantity() || !parseFloat(self.dogeBurnQuantity())) return null;
    return testnetBurnDetermineEarned(WALLET.networkBlockHeight(), self.dogeBurnQuantity());
  }, self);

  self.dispQuantityXDPToBeCreated = ko.computed(function() {
    return numberWithCommas(self.quantityXDPToBeCreated());
  }, self);
  ...
  self.validationModel = ko.validatedObservable({
    dogeBurnQuantity: self.dogeBurnQuantity
  });

  self.resetForm = function() {
    self.dogeBurnQuantity('');
    self.validationModel.errors.showAllMessages(false);
  }
  ...
  self.doAction = function() {
    ...
    WALLET.doTransaction(self.address(), "create_burn",
      {
        source: self.address(),
        quantity: denormalizeQuantity(self.dogeBurnQuantity())
      },
      function(txHash, data, endpoint, addressType, armoryUTx) {
        self.shown(false);
        ...
        var message;
        if (armoryUTx) {
          message = i18n.t("you_will_be_burning", self.dogeBurnQuantity(), self.quantityXDPToBeCreated());
        } else {
          message = i18n.t("you_have_burned", self.dogeBurnQuantity(), self.quantityXDPToBeCreated());
        }
        WALLET.showTransactionCompleteDialog(message + " " + i18n.t(ACTION_PENDING_NOTICE), message, armoryUTx);
      }
    );
  ...
  self.show = function(address, resetForm) {
    ...
    //get the current block height, to calculate the XDP burn payout
    ...
    failoverAPI("get_burns", {filters: {'field': 'source', 'op': '==', 'value': address}}, function(data, endpoint) {
      ...
      self.dogeAlreadyBurned(normalizeQuantity(totalBurned));
      self.shown(true);
...
function SignTransactionModalViewModel() {
  ...
  self.signTransaction = function() {
    assert(self.validationModel.isValid(), "Cannot sign");
    var dwk = WALLET.getAddressObj(self.address()).KEY;
    var signedHex = '';
    try {

      DWBitcore.signRawTransaction(self.unsignedTx(), dwk, true, function(err, signedHex) {
        if (err) {
