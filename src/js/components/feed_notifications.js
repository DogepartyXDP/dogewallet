...
NotificationViewModel.calcText = function(category, message) {
  ...
  if (category == "sends") {
  ...
  } else if (category == "dogepays" && (WALLET.getAddressObj(message['source']) || WALLET.getAddressObj(message['destination']))) {
    desc = i18n.t("notif_dogepay_from", getAddressLabel(message['source']), getAddressLabel(message['destination']),
      smartFormat(normalizeQuantity(message['doge_amount'])));
    ...
    // NOTE that dogepartyd has automatically already adusted the balances of all asset holders...we just need to notify
    ...
    if (message['transfer']) {
      ...
      if (addresses.indexOf(message['source']) != -1 || addresses.indexOf(message['issuer']) != -1) {
        desc = i18n.t("notif_token_transferred", message['_asset_longname'] || message['asset'], getLinkForDPData('address', message['source'], getAddressLabel(message['source'])),
          getLinkForDPData('address', message['issuer'], getAddressLabel(message['issuer'])));
      }
