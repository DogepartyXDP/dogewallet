function AddressViewModel(type, key, address, initialLabel, pubKeys) {
  ...
  self.assets = ko.observableArray([
    new AssetViewModel({address: address, asset: KEY_ASSET.DOGE}), //will be updated with data loaded from insight
    new AssetViewModel({address: address, asset: KEY_ASSET.XDP})  //will be updated with data loaded from dogepartyd
  ]);
  ...
  self.filteredAssets = ko.computed(function() {
    if (self.assetFilter() == '') { //show all
      ...
      return ko.utils.arrayFilter(self.assets(), function(asset) {
        return asset.ASSET === KEY_ASSET.DOGE || asset.ASSET === KEY_ASSET.XDP;
      });
  ...
  self.addOrUpdateAsset = function(asset, assetInfo, initialRawBalance, escrowedBalance) {
    ...
    if (asset === KEY_ASSET.DOGE || asset === KEY_ASSET.XDP) { //special case update
  ...
  self.createAsset = function() {
    ...
    var xdpBalance = WALLET.getBalance(self.ADDRESS, KEY_ASSET.XDP);
    CREATE_ASSET_MODAL.show(self.ADDRESS, xdpBalance, true);
  }
  ...
  self.showBaseAssetsOnly = function() {
    self.assetFilter('base'); //Show XDP and DOGE only
  }
  ...
  self.getXDPBalance = function() {
    var xdpAsset = $.grep(self.assets(), function(value) {
      return value.ASSET === KEY_ASSET.XDP;
    });
    return xdpAsset[0].normalizedBalance();
  }
