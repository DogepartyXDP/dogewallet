...
function CreateAssetModalViewModel() {
  ...
  self.xdpBalance = ko.observable(0);
  ...
  self.hasXDPForNamedAsset = ko.computed(function() {
    return self.xdpBalance() >= ASSET_CREATION_FEE_XDP;
  });
  self.hasXDPForSubAsset = ko.computed(function() {
    return self.xdpBalance() >= SUBASSET_CREATION_FEE_XDP;
  });
...
/* // this was on a conflict merge
    ...
    WALLET.doTransaction(self.address(), "create_issuance",
      ...
      function(txHash, data, endpoint, addressType, armoryUTx) {
        ...
        if (self.tokenNameType() == 'alphabetic') {
          message += i18n.t("issuance_end_message", getAddressLabel(self.address()), ASSET_CREATION_FEE_XDP);
        } else {
  ...
  self.show = function(address, xdpBalance, resetForm) {
    if (typeof(resetForm) === 'undefined') resetForm = true;
    if (resetForm) self.resetForm();
    self.xdpBalance(xdpBalance);
...
function PayDividendModalViewModel() {
  ...
  self.doAction = function() {
    ...
    if (self.selectedDividendAsset() === KEY_ASSET.DOGE) {
      var params = {
        'filters': [
  ...
  self.showModal = function(address, resetForm) {
    ...
    failoverAPI("get_normalized_balances", {'addresses': [address.ADDRESS]}, function(data, endpoint) {
      ...
      WALLET.retrieveDOGEBalance(address.ADDRESS, function(balance) {
        if (balance) {
          self.availableDividendAssets.unshift(new DividendAssetInDropdownItemModel(KEY_ASSET.DOGE, KEY_ASSET.DOGE, balance, normalizeQuantity(balance)));
        }
      });
    });
  }
