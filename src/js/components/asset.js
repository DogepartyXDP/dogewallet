function AssetViewModel(props) {
  //An address has 2 or more assets (DOGE, XDP, and any others)
  var self = this;
  ...
  self.rawBalance = ko.observable(props['rawBalance'] || (self.ASSET === KEY_ASSET.DOGE ? null : 0));
  //^ raw (not normalized) (for DOGE/XDP, default to null to show '??' instead of 0, until the balance is populated)
  ...
  self.isDOGE = ko.computed(function() {
    return self.ASSET === KEY_ASSET.DOGE;
  }, self);

  self.isXDP = ko.computed(function() {
    return self.ASSET === KEY_ASSET.XDP;
  }, self);
