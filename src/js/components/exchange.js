...
function createExchangeKnockoutValidators() {
  ko.validation.rules['ordersIsExistingAssetName'] = {
    validator: function(asset, self) {
      if (asset === KEY_ASSET.XDP) return true;
...
/*      _fee_option: self.sellFeeOption(),
  ...
  self.sellFeeController = DWFeeModelMixin(self, {
    prefix: 'sell_',
    action: "create_order",
...
*/
  ...
  self.sell = function() {
    ...
    message += '<tr><td><b>' + i18n.t('fee') + ': </b></td><td style="text-align:right">' + self.sellFeeController.getFeeInDOGE() + '</td><td>' + KEY_ASSET.DOGE + ' ('+self.sellFeeController.getFeeInFiat()+' ' + KEY_ASSET.USD + ')</td></tr>';
    message += '</table>';
  ...
  // mix in shared fee calculation functions
  self.buyFeeController = DWFeeModelMixin(self, {
    prefix: 'buy_',
    action: "create_order",
  ...
  self.buy = function() {
    ...
    message += '<tr><td><b>' + i18n.t('real_estimated_total') + ': </b></td><td style="text-align:right">' + estimatedTotalPrice + '</td><td>' + self.dispQuoteAsset() + '</td></tr>';
    message += '<tr><td><b>' + i18n.t('fee') + ': </b></td><td style="text-align:right">' + self.buyFeeController.getFeeInDOGE() + '</td><td>' + KEY_ASSET.DOGE +' ('+self.buyFeeController.getFeeInFiat()+' ' + KEY_ASSET.USD + ')</td></tr>';
    message += '</table>';
  ...
  self.selectMarket = function(item) {
    ...
    if (item.quote_asset === KEY_ASSET.XDP) {
  ...
  self.cancelOrder = function(order) {
    ...
    if (WALLET.cancelOrders.indexOf(order.tx_hash) != -1) {
      ...
      var message = i18n.t('cancel_consume_doge');
