function MessageFeed() {
  var self = this;
  self.lastMessageIndexReceived = ko.observable(0); //message index = the message feed index generated by dogeparty-server (mempool tx do not have indexes and indexes CAN shift back during reorgs)
  self.lastMessageSeqReceived = ko.observable(0); //message seq = the message feed sequence number generated by dogeblock's dogewallet module (mempool tx DO have indexes, and seqs DO NOT shift back during reorgs)
  self.OPEN_ORDERS = []; // here only for sellDOGEOrdersCount
  self.POLL_INTERVAL = 30000; // poll the server every 30 seconds

  self.init = function(last_message_index, dw_last_message_seq) {
    self.lastMessageIndexReceived(last_message_index);
    self.lastMessageSeqReceived(dw_last_message_seq);
  ...
  self.restoreOrder = function() {
    ...
    failoverAPI("get_orders", {'filters': filters, 'show_expired': false, 'filterop': 'or'},
      function(data, endpoint) {
        //do not show empty/filled orders, including open DOGE orders that have 0/neg give remaining 
  ...
  self.parseMempoolTransaction = function(txHash, category, message) {
    ...
    if (displayTx) {
      ...
      /*var asset1 = message['bindings']['asset'] || 'DOGE';
  ...
  self.parseMessage = function(seq, when, message) {
    ...
    //NOTE: we use the message sequence that the dogeblock dogewallet module generates, as it takes into account mempool transactions
    // (which come across the dogeblock dogewallet feed like any other message)
    // the message_index created by the dogeparty-server message feed DOES NOT generate message_indexes for mempool transactions
    ...
    assert(self.lastMessageIndexReceived() + 1 == message['_message_index'], "Message feed resync doge increment oddity...?");
    ...
    if (message['_command'] == 'reorg') {
      ...
      setTimeout(function() { WALLET.refreshDogepartyBalances(WALLET.getAddressesList(), checkURL); }, randomIntFromInterval(1, 5) * 1000);
    ...
    // just relying on 5 minute polling for new DOGE balances)
    ...
    if (category == "balances") {
      ...
      if (message['_doge_below_dust_limit'])
        return; //ignore any order involving DOGE below the dust limit
      ...
      if (message['_doge_below_dust_limit'])
        return; //ignore any order match involving DOGE below the dust limit
