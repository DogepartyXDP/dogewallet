...
/***********
 * DOGEWALLET.CONF.JSON LOADING AND SERVICES INITIALIZATION
 ***********/
var dwURLs = ko.observableArray([]);
var dwBaseURLs = ko.observableArray([]);
var dwAPIUrls = ko.observableArray([]);
var disabledFeatures = ko.observableArray([]);
var restrictedAreas = {};

function produceDWServerList() {
  dwURLs(_.shuffle(dwURLs())); //randomly shuffle the list to decide the server try order...
  $.jqlog.debug("MultiAPI Backends: " + JSON.stringify(dwURLs()));

  dwBaseURLs(jQuery.map(dwURLs(), function(element) {
    return element;
  }));
  dwAPIUrls(jQuery.map(dwURLs(), function(element) {
    if (USE_TESTNET) {
...
function initRollbar() {
  /* TODO: Try to load rollbar earlier, possibly... (However, as we get the accessToken from dogewallet.conf.json, we'd have
...
function loadDogewalletConfigFromServer() {
  //Request for the dogewallet.conf.json file, which should contain an array of API backends for us to use
  $.getJSON(DOGEWALLET_CONF_LOCATION, function(data) {
    assert(data && typeof data == "object" && data.hasOwnProperty("servers"), "Returned dogewallet.conf.json file does not contain valid JSON object");
    assert(data['servers'] && data['servers'] instanceof Array, "'servers' field in returned dogewallet.conf.json file is not an array");
    ...
    if (data['disabledFeatures']) {
      assert(data['disabledFeatures'] instanceof Array, "'disabledFeatures' field in returned dogewallet.conf.json file is not an array");
    ...
    if (data['restrictedAreas']) {
      assert(data['restrictedAreas'] instanceof Object, "'restrictedAreas' field in returned dogewallet.conf.json file is not an object");
...
$(document).ready(function() {
  ...
  loadDogewalletConfigFromServer();
