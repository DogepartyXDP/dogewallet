// usage:
//   var feeController = DWFeeModelMixin(modalDialogModel)
//   feeController.reset()
var DWFeeModelMixin = function(modalDialogModel, opts) {
  var exports = {};
  ...
  exports.getFeeText = function() {
    var args = getObs('feeDetailsLocaleArgs')
    return args[0]+' ' + KEY_ASSET.DOGE + ' ('+args[1]+' '+KEY_ASSET.USD + ')';
  }
  ...
  function init() {
    ...
    obsObj('feeSlider').subscribe(function(sliderOffset) {
      DWDogecoinFees.getFeeByOffset(sliderOffset, function(fee) {
  ...
  var setFeeDefaults = function() {
    DWDogecoinFees.getFees(function(fees) {
      setObs('feeSliderMax', fees.length - 1);
      DWDogecoinFees.defaultFee(function(fee) {
  ...
  var feeParametersChanged = function() {
    ...
    DWDogecoinFees.getFeeByOffset(getObs('feeSlider'), function(fee) {
      if (fee.maxDelay == 0) {
  ...
  var debouncedFeeParametersChanged = _.debounce(function() {
    ...
    WALLET.createUnsignedTransactionWithExtendedTXInfo(opts.address(), opts.action, tx_data,
      function(extendedTxInfo, offset) {
        ...
        var feeFloat = extendedTxInfo.doge_fee / UNIT

        DWDogecoinQuote.getQuote(function(quote) {
          var fiatString = formatFiat(feeFloat * quote, 2, 2)

          var hasDOGEOut = extendedTxInfo.doge_out > 0;
          var isDOGESend = false;
          if (hasDOGEOut && opts.action == "create_send" && tx_data.asset === KEY_ASSET.DOGE) {
            isDOGESend = true;
          }

          if (hasDOGEOut && !isDOGESend) {
            var dustFloat = extendedTxInfo.doge_out / UNIT
            setObs('feeDetailsLocaleName', 'fee_details_with_doge_out');
